...
// Tab definitions — counts are computed dynamically in the component
...
export default function LeadGenList({ leads: propLeads, loading, onRefresh, toast, onSelectLead, onNavigate }) {
  ...
  // Compute real counts from leads array
  const totalLeads = leads.length;
  const hotLeads = leads.filter(l => l.hot || l.score >= 80);
  const hotCount = hotLeads.length;
  const warnLeads = leads.filter(l => l.warn);
  const warnCount = warnLeads.length;
  const pipelineCount = leads.filter(l => l.score >= 65 && !l.warn).length;
  const grapevineCount = leads.filter(l => l.source?.toLowerCase().includes('grapevine')).length;
  const archivedCount = leads.filter(l => l.score < 50).length;
        ...
        <div style={S.pageWrap}>
          ...
          <div style={S.pageHeader}>
            <div>
              ...
              <div style={S.pageSub}>{totalLeads} leads · {hotCount} hot (A+/A) · {warnCount} active WARN signals</div>
            </div>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              <button style={{ ...S.btn, background: 'var(--rust)', color: '#fff', borderColor: 'var(--rust)', fontSize: 12 }} onClick={() => setActiveTab('hot')}>⚡ Hot Queue ({hotCount})</button>
          ...
          <div style={S.kpiStrip}>
            {[
              { icon: '◉', label: 'Total Leads', val: totalLeads, color: 'blue' },
              { icon: '⚡', label: 'Hot — A+/A', val: hotCount, color: 'rust' },
              { icon: '⚠', label: 'WARN Matched', val: warnCount, color: 'amber' },
              { icon: '◈', label: 'Pipeline Candidates', val: pipelineCount, color: 'green' },
              { icon: '↗', label: 'Grapevine', val: grapevineCount, color: 'purple' },
            ].map((k, i) => (
          ...
          <div style={S.tabsNav}>
            {[
              { label: 'All Leads', key: 'all', count: totalLeads },
              { label: 'A+/A Hot', key: 'hot', count: hotCount, hot: true },
              { label: 'WARN Matched', key: 'warn', count: warnCount, hot: true },
              { label: 'Pipeline Ready', key: 'pipeline', count: pipelineCount },
              { label: 'Grapevine', key: 'grapevine', count: grapevineCount },
          ...
          <div style={S.filterBar}>
            ...
            <span style={{ marginLeft: 'auto', fontSize: 12, color: 'var(--ink4)' }}>Showing <strong style={{ color: 'var(--ink2)' }}>{filteredLeads.length}</strong> of {totalLeads}</span>
          </div>
