...
/* ═══════════════════════════════════════════════════════════
   ...
   v4 rebuild — April 2026
   ① Single "Scores" cell: Fit (green) · Bldg (blue) · ORS (rust)
   ② SF rounded to K, Land 1 decimal, Coverage whole number
   ③ Property tags (purple) separated from catalyst tags with divider
   ④ Status column compact
   ⑤ Column alignment matched to headers
   ═══════════════════════════════════════════════════════════ */
...
const fmtCov = (sf, acres) => {
  if (!sf || !acres) return '—';
  return `${Math.round((sf / (acres * 43560)) * 100)}%`;
};
...
// ── TAG CLASSIFICATION ────────────────────────────────────
// Property tags = building characteristics (purple)
const PROPERTY_TAG_KEYWORDS = ['capex', 'vintage', 'low clear', 'coverage', 'multi-parcel', 'roof', 'environmental', 'rezoning', 'high office', 'no sprinklers', 'small truck court', 'grade-level only', 'rail'];
// Catalyst tags = owner/occupancy/financial/market signals
const isBuildingTag = (tag) => {
  const t = (tag || '').toLowerCase();
  return PROPERTY_TAG_KEYWORDS.some(kw => t.includes(kw));
};

const getCatalystStyle = (tag) => {
  if (!tag) return { bg: CL.blueBg, bdr: CL.blueBdr, c: CL.blue };
  const t = tag.toLowerCase();
  if (t.includes('warn') || t.includes('owner-user') || t.includes('owner age') || t.includes('owner distress') || t.includes('nod filed') || t.includes('bankruptcy') || t.includes('legacy hold') || t.includes('long hold') || t.includes('expired') || t.includes('prior listing') || t.includes('no broker')) return { bg: CL.rustBg, bdr: CL.rustBdr, c: CL.rust };
  if (t.includes('lease') || t.includes('partial') || t.includes('below market') || t.includes('vacant') || t.includes('lease-up') || t.includes('tenant credit') || t.includes('downsizing')) return { bg: CL.amberBg, bdr: CL.amberBdr, c: CL.amber };
  ...
  if (t.includes('institutional') || t.includes('investment grade') || t.includes('private llc') || t.includes('estate') || t.includes('trust') || t.includes('debt')) return { bg: CL.blueBg, bdr: CL.blueBdr, c: CL.blue };
  return { bg: CL.blueBg, bdr: CL.blueBdr, c: CL.blue };
};

const getPropertyTagStyle = () => ({ bg: CL.purpleBg, bdr: CL.purpleBdr, c: CL.purple });
...
export default function PropertiesPage() {
  ...
  // ── TABLE COLUMNS ───────────────────────────────────────
  // Single "Scores" column replaces 3 separate score columns
  const cols = [
    { key: 'property_name', label: 'Property', w: 'auto' },
    { key: 'fit_score', label: 'Scores', w: 170 },
    { key: 'building_sf', label: 'SF', w: 72 },
    { key: 'clear_height', label: 'Clr', w: 52 },
    { key: 'land_acres', label: 'Land', w: 56 },
    { key: null, label: 'Cov', w: 52 },
    { key: 'year_built', label: 'Year', w: 56 },
    { key: 'owner', label: 'Owner', w: 130 },
    { key: null, label: 'Status', w: 78 },
    ...
    <>
      ...
      <div style={{ padding: '28px 0 20px', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <div style={{ display: 'flex', background: CL.bg2, border: `1px solid ${CL.line}`, borderRadius: 7, overflow: 'hidden' }}>
            ...
            <button onClick={() => alert('Map view coming in next build.')} style={{ padding: '7px 14px', fontSize: 12, fontWeight: 500, cursor: 'pointer', border: 'none', background: 'transparent', color: CL.ink4, fontFamily: "'Instrument Sans',sans-serif" }}>🗺 Map</button>
            <button onClick={() => alert('Cards view coming in next build.')} style={{ padding: '7px 14px', fontSize: 12, fontWeight: 500, cursor: 'pointer', border: 'none', background: 'transparent', color: CL.ink4, fontFamily: "'Instrument Sans',sans-serif" }}>◫ Cards</button>
          </div>
      ...
      <div style={{ background: CL.card, borderRadius: 12, boxShadow: CL.shadow, border: `1px solid ${CL.line2}`, overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
          <colgroup>
            <col style={{ width: 36 }} />
            {cols.map((col, i) => <col key={i} style={{ width: col.w === 'auto' ? undefined : col.w }} />)}
            <col style={{ width: 24 }} />
          </colgroup>
          <thead><tr>
            <th style={{ padding: '11px 10px', borderBottom: `2px solid ${CL.line}`, background: CL.bg }}>
            ...
            {cols.map((col, i) => (
              <th key={i} onClick={() => col.key && handleSort(col.key)} style={{
                padding: '11px 10px', textAlign: 'left', fontSize: 11, fontWeight: 600,
            ...
            {loading ? <tr><td colSpan={cols.length + 2} style={{ padding: 48, textAlign: 'center', color: CL.ink4, fontSize: 16 }}>Loading properties…</td></tr> :
              filtered.length === 0 ? <tr><td colSpan={cols.length + 2} style={{ padding: 48, textAlign: 'center', color: CL.ink4, fontSize: 16 }}>No properties match filters.</td></tr> :
                filtered.map((p, idx) => {
                  ...
                  const cov = fmtCov(p.building_sf, p.land_acres);
                  const allPTags = (p.catalyst_tags || []);
                  const propTags = allPTags.filter(t => isBuildingTag(t));
                  const catTags = allPTags.filter(t => !isBuildingTag(t));
                  return (
                    <tr key={p.id} onClick={() => router.push(`/properties/${p.id}`)}
                      ...
                      <td style={{ padding: '10px 10px', verticalAlign: 'middle', overflow: 'hidden' }}>
                        <div style={{ fontWeight: 600, color: CL.ink, fontSize: 14, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{p.property_name || p.address || '—'}</div>
                        <div style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: 'italic', fontSize: 13, color: CL.ink4, marginTop: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{[p.city, p.submarket].filter(Boolean).join(' · ') || '—'}</div>
                      </td>
                      {/* ① SINGLE SCORES CELL — Fit (green) · Bldg (blue) · ORS (rust) */}
                      <td style={{ padding: '8px 10px', verticalAlign: 'middle' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          {/* Fit ring — green scale */}
                          <ScoreRing
                            value={p.fit_score}
                            label={p.fit_grade || getGrade(p.fit_score)}
                            color={p.fit_score >= 65 ? CL.green : p.fit_score >= 35 ? CL.amber : CL.ink4}
                            bg={p.fit_score >= 65 ? CL.greenBg : p.fit_score >= 35 ? CL.amberBg : 'rgba(0,0,0,0.03)'}
                            bdr={p.fit_score >= 65 ? CL.greenBdr : p.fit_score >= 35 ? CL.amberBdr : CL.line}
                            glow={p.is_acq_target}
                            idx={idx}
                          />
                          {/* Bldg score — blue scale, number only */}
                          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: 32 }}>
                            <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 14, fontWeight: 700, color: p.ai_score >= 70 ? CL.blue : p.ai_score >= 55 ? CL.amber : CL.ink4, lineHeight: 1 }}>{p.ai_score ?? '—'}</span>
                            <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 8, color: CL.ink4, marginTop: 1 }}>Bldg</span>
                          </div>
                          {/* Separator dot */}
                          <span style={{ width: 2, height: 2, borderRadius: '50%', background: CL.line, flexShrink: 0 }} />
                          {/* ORS — rust scale, number only */}
                          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: 32 }}>
                            <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 14, fontWeight: 700, color: p.probability >= 75 ? CL.rust : p.probability >= 50 ? CL.amber : p.probability >= 25 ? CL.blue : CL.ink4, lineHeight: 1 }}>{p.probability ?? '—'}</span>
                            <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 8, color: CL.ink4, marginTop: 1 }}>ORS</span>
                          </div>
                          {p.is_acq_target && <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 8, fontWeight: 700, color: CL.green, background: CL.greenBg, border: `1px solid ${CL.greenBdr}`, borderRadius: 3, padding: '1px 4px', flexShrink: 0 }}>ACQ</span>}
                        </div>
                      </td>
                      {/* ② SF rounded to K */}
                      ...
                      <td style={{ padding: '10px 10px', fontFamily: "'DM Mono',monospace", fontSize: 13, color: CL.ink2, verticalAlign: 'middle' }}>{cov}</td>
                      {/* Year */}
                      ...
                      <td style={{ padding: '8px 6px', verticalAlign: 'middle' }}>
                        <StatusDot status={p.vacancy_status} />
                      </td>
                      {/* ③ Tags — property (purple) + catalyst (colored), visual divider */}
                      <td style={{ padding: '8px 10px', verticalAlign: 'middle' }}>
                        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', alignItems: 'center' }}>
                          {/* Catalyst tags first (signals) */}
                          {catTags.slice(0, 2).map((tag, i) => {
                            const s = getCatalystStyle(tag);
                            const urgent = isUrgentTag(tag);
                            return <span key={`c${i}`} className={urgent ? 'catalyst-pulse' : ''} style={{ display: 'inline-flex', padding: '2px 7px', borderRadius: 4, fontSize: 10.5, fontWeight: 600, background: s.bg, border: `1px solid ${s.bdr}`, color: s.c, whiteSpace: 'nowrap' }}>{tag}</span>;
                          })}
                          {catTags.length > 2 && <span style={{ fontSize: 10, color: CL.ink4, fontFamily: "'DM Mono',monospace" }}>+{catTags.length - 2}</span>}
                          {/* Divider between catalyst and property tags */}
                          {catTags.length > 0 && propTags.length > 0 && (
                            <span style={{ width: 1, height: 14, background: CL.line, margin: '0 2px', flexShrink: 0 }} />
                          )}
                          {/* Property tags (purple, building characteristics) */}
                          {propTags.slice(0, 1).map((tag, i) => {
                            const s = getPropertyTagStyle();
                            return <span key={`p${i}`} style={{ display: 'inline-flex', padding: '2px 7px', borderRadius: 4, fontSize: 10.5, fontWeight: 600, background: s.bg, border: `1px solid ${s.bdr}`, color: s.c, whiteSpace: 'nowrap' }}>{tag}</span>;
                          })}
                          {propTags.length > 1 && <span style={{ fontSize: 10, color: CL.purple, fontFamily: "'DM Mono',monospace" }}>+{propTags.length - 1}</span>}
                          {p.ai_synthesis && <AISparkle text={p.ai_synthesis} />}
                        </div>
                      </td>
                      {/* Chevron */}
                      <td style={{ padding: '10px 4px', color: CL.ink4, fontSize: 14, opacity: 0.5, verticalAlign: 'middle' }}>›</td>
                    </tr>
                  );
                })}
          </tbody>
          ...
          {compareProps.length === 0 ? (
            ...
            <div style={{ flex: 1, overflowY: 'auto' }}>
              ...
              {[
                ...
                { label: 'Coverage', fn: p => fmtCov(p.building_sf, p.land_acres) },
          ...
          <div style={{ background: '#fff', borderRadius: 14, boxShadow: CL.shadowMd, padding: 28, width: 440, maxWidth: '90vw' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 20, maxHeight: 120, overflowY: 'auto' }}>
              {allTags.slice(0, 20).map(tag => {
                const s = isBuildingTag(tag) ? getPropertyTagStyle() : getCatalystStyle(tag);
...
function ScoreRing({ value, label, color, bg, bdr, glow, idx = 0 }) {
  if (value == null) return <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 12, color: '#6E6860' }}>—</span>;
  return (
    <div className={glow ? 'score-ring fit-ring-glow' : 'score-ring'} style={{
      width: 36, height: 36, borderRadius: '50%',
      border: `2.5px solid ${bdr}`, background: bg,
      display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
      animation: 'scoreIn 0.5s ease both', animationDelay: `${idx * 30}ms`,
      flexShrink: 0,
    }}>
      <span style={{ fontFamily: "'Playfair Display',serif", fontSize: 14, fontWeight: 700, color, lineHeight: 1 }}>{value}</span>
      <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 7.5, color, marginTop: 1 }}>{label}</span>
    </div>
  );
}
...
// ④ Compact status — dot + short label instead of full badge
function StatusDot({ status }) {
  const s = (status || '').toLowerCase();
  let label = '—', dot = CL.ink4;
  if (s.includes('occupied') || s === 'leased') { label = 'Occ'; dot = CL.green; }
  else if (s.includes('vacant') || s === 'available') { label = 'Vac'; dot = CL.rust; }
  else if (s.includes('partial')) { label = 'Prt'; dot = CL.amber; }
  else if (s === 'active' || s === 'listed') { label = 'List'; dot = CL.blue; }
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
      <span style={{ width: 7, height: 7, borderRadius: '50%', background: dot, flexShrink: 0 }} />
      <span style={{ fontSize: 11.5, fontWeight: 600, color: dot }}>{label}</span>
    </div>
  );
}
